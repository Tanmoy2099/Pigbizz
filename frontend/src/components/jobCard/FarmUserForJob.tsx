...
const limit = 9
const FarmUserForJob = (props: any) => {
  ...
  const tableData = pageData.map((val: any) => <React.Fragment key={val}> <tr>
    <td>{val?.assignee ? val.assignee.name : "System Genetared"}</td>
    <td>{val?.task}</td>
    {/* TODO: action for the task */}
    <td>Action button</td>
  </tr>
  </React.Fragment>)
  ...
  //   if (searchTerm.length === 0 && value.length === 1) setPagination((val: any) => ({ ...val, page: 0 }));
  //   setSearchTerm(value);
  ...
  return (
    <>
      ...
      <div className="w-full flex justify-center my-5">
        <div className="btn-group m-auto">
          {pagination.page !== 0 &&
            <Button type="button" className="btn_page_lg btn-primary" onClick={() => setPagination((val: any) => ({ ...val, page: 0 }))}>«</Button>}

          {pagination.page > 1 && <Button type="button" className="btn_page_lg btn-primary" onClick={() => setPagination((val: any) => ({ ...val, page: val.page - 2 }))}>
            {pagination.page - 1}
          </Button>}

          {pagination.page > 2 && <Button type="button" className="btn_page_lg btn-primary" onClick={() => setPagination((val: any) => ({ ...val, page: val.page - 1 }))}>
            {pagination.page}
          </Button>}
          ...
          {(pagination.page < lastpage - 2) && <Button type="button" className="btn_page_lg btn-primary" onClick={() => setPagination((val: any) => ({ ...val, page: lastpage - 2 }))}>{lastpage - 1}</Button>}

          {(pagination.page < lastpage - 1) && <Button type="button" className="btn_page_lg btn-primary"
            onClick={() => setPagination((val: any) => ({ ...val, page: lastpage }))}
          >{lastpage - 1}</Button>}
